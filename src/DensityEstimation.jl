...
function getEyPy(sims::Array, X_grid::Matrix{T},
    ...
    R == (length(sims[1][:lλ])-1) == length(D) == size(X,2) || throw("R is inconsistent in one of the args.")

    nsim = length(sims)

    Dstar = [ pairDistMat( X_grid[:,[j]] ) for j = 1:R ]
    Dstdat = [ pairDistMat( X_grid[:,[j]],  X[:,[j]] ) for j = 1:R ]

    Fstar = Array{Float64}(undef, R, n_star, nsim)
    ...
    for ii = 1:nsim
        for i = 1:n_star
            ...
            λ = exp.(sims[ii][:lλ])
            Ey[i,ii] = λ[1] * sims[ii][:intercept].μ + sum( λ[2:(R+1)] .* Fstar[:,i,ii] )
